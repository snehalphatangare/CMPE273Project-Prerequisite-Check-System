...
def doOCR():
    try:
        ...
        enrollmentType = data['hrefEnrollType'][0]

        print enrollmentType


        #save to image file
        fh = open('/Users/Abhishek/Sem1Projects/CMPE273Project/PrereqCheckSystem/server/imageToSave.png', 'wb')
        fh.write(decodestring(str(img_data)))
        fh.close()

        #do OCR on the saved image
        fh = open('/Users/Abhishek/Sem1Projects/CMPE273Project/PrereqCheckSystem/server/imageToSave.png', 'r')
        result = image_to_string(Image.open(fh), lang='eng')
        file = open('/Users/Abhishek/Sem1Projects/CMPE273Project/PrereqCheckSystem/server/output.txt','w')
        file.write(result) 
        file.close()
        fh.close()
        #return 'Success!'
        ...
        if(str(enrollmentType).__contains__('semesterEnrollment')):
        ...
        if(applicationType == "Masters"):
            ...
            if(isEligible):
                ...
                isEnrolled = saveDetailsToDB(masterProgrammeEnrolled, userEmail, enrollmentStatus)
                #send notification mail to the student from the dept chair
                if(isEnrolled):
                    sendNotificationMail(userEmail,"true",masterProgrammeEnrolled)
                ...
                print "inside master else"
                sendNotificationMail(userEmail,"false",masterProgrammeEnrolled)


        else:
            print "inside semester enroll"
            if(len(requestedSubjects)>0):
                ...
                if(len(eligibleCoursesList)>0):
                    ...
                    if(len(eligibleCourseProfMailList)>0):
                        for i in range(len(eligibleCourseProfMailList)):
                            if(eligibleCourseProfMailList[i]=='sithu.aung@sjsu.edu'):
                                ...
                                profEmail = "abhishek.upadhyay@sjsu.edu"
                    ...
                    allCourses = ''
                    for item in range(len(eligibleCoursesList)):
                        allCourses = allCourses + eligibleCoursesList[item]

                    saveDetailsToDB(applicationType, stuEmail, enrollmentStatus)
    ...
    #try :
        ...
        #print "Unexpected Exception Occured ", sys.exc_info()[0]
...
def saveDetailsToDB(masterProgrammeEnrolled, stuEmailId, enrollmentStatus):
    print "DB invoked"
    conn = sqlite3.connect('/Users/Abhishek/Sem1Projects/CMPE273Project/PrereqCheckSystem/server/EnrollmentDetailsDB.sqlite')
    c = conn.cursor()
    ...
    isEnrolled = "false"
    c.execute('SELECT * FROM EnrollmentDetailsDB')
    rows = c.fetchall()
    for row in rows:
        print row[0]
        if(row[0]==stuEmailId):
...
def sendNotificationMail(toaddr,isEligible, coursesList):
    print "mail notify invoked"
    # Connecting to the database file
    conn = sqlite3.connect('/Users/Abhishek/Sem1Projects/CMPE273Project/PrereqCheckSystem/server/CheckSyatemDB.sqlite')
    c = conn.cursor()

    c.execute('SELECT * FROM emailDB')
    dbRecordTuple = c.fetchone()
    adminEmailId = dbRecordTuple[0]
    print "Admin Id ",adminEmailId
    ...
    if(isEligible):
        if(len(coursesList)>0):
            ...
            body = "You Are Enrolled Successfully for the master's programmes !!"
    else:
...
def checkEligibility(applicationType,requestedSubjects):
    file = open('/Users/Abhishek/Sem1Projects/CMPE273Project/PrereqCheckSystem/server/output.txt', 'r')
...
def checkMastersEligibility(lstTranscriptLines):
    ...
    with open('/Users/Abhishek/Sem1Projects/CMPE273Project/PrereqCheckSystem/server/mastersApplicationConditions.json') as data_file:
...
def checkSubEnrollmentEligibility(lstTranscriptLines,requestedSubjects):
    ...
    with open('/Users/Abhishek/Sem1Projects/CMPE273Project/PrereqCheckSystem/server/subjectEnrollmentConditions.json') as data_file:
        conditionsData = json.load(data_file)
    print 'conditions data = ', conditionsData
    # Get the subject conditions for student
    subjectConditions = conditionsData['subjects']
    ...
    # Read the info file
    with open('/Users/Abhishek/Sem1Projects/CMPE273Project/PrereqCheckSystem/server/info.json') as data_file:
